...
{
	u64 aligned_start;
//early_print("**arm_add_memory() start start=%llxh, size=%llxh\n", start, size);////debug 80000000h 4000000h
...
//early_print("**arm_add_memory() start aligned_start=%lx\n", (unsigned long)aligned_start);////debug-80000000h
	...
	//**arm_add_memory() end start=80000000, size=80000000(-2147483648)
//early_print("**arm_add_memory() end start=%llxh, size=%llxh\n", start, size);////debug
	return 0;
}
...
void __init request_standard_resources(const struct machine_desc *mdesc)
{
...
////debug end
	...
	*cmdline_p = cmd_line;
	//early_print("**setup_arch() boot_command_line=%s\n", boot_command_line);////debug
	...
	psci_dt_init();
#ifdef CONFIG_SMP
early_print("setup_arch() CONFIG_SMP\n");////debug - not called
	if (is_smp()) {
