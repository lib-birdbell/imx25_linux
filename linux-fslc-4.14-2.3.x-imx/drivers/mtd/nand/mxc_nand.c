...
//printk(KERN_ERR "**check_int_v1_v2() tmp=%xh\n", tmp);////debug
	if (!(tmp & NFC_V1_V2_CONFIG2_INT))
		return 0;

	if (!host->devtype_data->irqpending_quirk){
//printk(KERN_ERR "**check_int_v1_v2() !quirk\n");////debug
		writew(tmp & ~NFC_V1_V2_CONFIG2_INT, NFC_V1_V2_CONFIG2);
	}////debug brace

//printk(KERN_ERR "**check_int_v1_v2() return 1\n");////debug
	return 1;
}
...
//printk(KERN_ERR "**send_cmd_v1_v2()\n");////debug
...
//printk(KERN_ERR "**send_cmd_v1_v2() break\n");////debug
				break;
			}
			udelay(1);
		}
		if (max_retries < 0){
//printk(KERN_ERR "**send_cmd_v1_v2() RESET failed\n");////debug
			pr_debug("%s: RESET failed\n", __func__);
		}////debug brace
	} else {
//printk(KERN_ERR "**send_cmd_v1_v2() wait_op_done()\n");////debug
		/* Wait for operation to complete */
		wait_op_done(host, useirq);
	}
//printk(KERN_ERR "**send_cmd_v1_v2() end\n");////debug
}
...
{
	//printk(KERN_ERR "**send_addr_v1_v2()\n");////debug
	pr_debug("send_addr(host, 0x%x %d)\n", addr, islast);

	writew(addr, NFC_V1_V2_FLASH_ADDR);
	writew(NFC_ADDR, NFC_V1_V2_CONFIG2);

	//printk(KERN_ERR "**send_addr_v1_v2() wait_op_done\n");////debug
	/* Wait for operation to complete */
	wait_op_done(host, islast);
	//printk(KERN_ERR "**send_addr_v1_v2() end\n");////debug
}
...
static void mxc_nand_select_chip_v2(struct mtd_info *mtd, int chip)
{
	struct nand_chip *nand_chip = mtd_to_nand(mtd);
	struct mxc_nand_host *host = nand_get_controller_data(nand_chip);
//printk(KERN_ERR "**mxc_nand_select_chip_v2()\n");////debug
	if (chip == -1) {
//printk(KERN_ERR "**mxc_nand_select_chip_v2() disable NFC clock\n");////debug
		/* Disable the NFC clock */
		if (host->clk_act) {
			clk_disable_unprepare(host->clk);
			host->clk_act = 0;
		}
//printk(KERN_ERR "**mxc_nand_select_chip_v2() disable NFC clock return\n");////debug
		return;
	}

//printk(KERN_ERR "**mxc_nand_select_chip_v2() !host->clk=%ph\n", (void*)&host->clk);////debug
	if (!host->clk_act) {
		/* Enable the NFC clock */
		clk_prepare_enable(host->clk);
		host->clk_act = 1;
//printk(KERN_ERR "**mxc_nand_select_chip_v2() !host->clk end\n");////debug
	}
...
static void mxc_do_addr_cycle(struct mtd_info *mtd, int column, int page_addr)
{
	struct nand_chip *nand_chip = mtd_to_nand(mtd);
	struct mxc_nand_host *host = nand_get_controller_data(nand_chip);
//printk(KERN_ERR "**mxc_do_addr_cycle() c=%d\n", column);////debug
	/* Write out column address, if necessary */
	if (column != -1) {
//printk(KERN_ERR "**mxc_do_addr_cycle() 1 send_addr mtd->writesize=%d\n", mtd->writesize);////debug
...
//printk(KERN_ERR "**mxc_do_addr_cycle() page_addr=%xh\n", page_addr);////debug
	/* Write out page address, if necessary */
	if (page_addr != -1) {
//printk(KERN_ERR "**mxc_do_addr_cycle() 2 send_addr mtd->writesize=%d\n", mtd->writesize);////debug
		/* paddr_0 - p_addr_7 */
...
//printk(KERN_ERR "**mxc_do_addr_cycle() end\n");////debug
}
					...
					const struct nand_data_interface *conf)
{
	...
	u16 config1;
printk(KERN_ERR "**mxc_nand_v2_setup_data_interface() NFC_V1_CONFIG1=%xh\n", NFC_V1_V2_CONFIG1);////debug
	timings = nand_get_sdr_timings(conf);
//printk(KERN_ERR "**mxc_nand_v2_setup_data_interface() timings=%ph\n", timings);////debug
	if (IS_ERR(timings))
		return -ENOTSUPP;

	config1 = readw(NFC_V1_V2_CONFIG1);

//printk(KERN_ERR "**mxc_nand_v2_setup_data_interface() config1=%xh\n", config1);////debug - c1bh
	tRC_min_ns = timings->tRC_min / 1000;
	rate = 1000000000 / tRC_min_ns;
//printk(KERN_ERR "**mxc_nand_v2_setup_data_interface() rate=%lu, tRC_min_ns=%d\n", rate, tRC_min_ns);////debug - 100000000, 100
	...
	if (tRC_min_ns < 30) {
		...
		rate *= 2;
		rate_round = clk_round_rate(host->clk, rate);
		printk(KERN_ERR "**mxc_nand_v2_setup_data_interface() rate_round=%lu, rate=%lu\n", rate_round, rate);////debug
		config1 &= ~NFC_V2_CONFIG1_ONE_CYCLE;
...
printk(KERN_ERR "**mxc_nand_v2_setup_data_interface() EINVAL\n");////debug
		return -EINVAL;
	}

printk(KERN_ERR "**mxc_nand_v2_setup_data_interface() csline=%d\n", csline);////debug
	if (csline == NAND_DATA_IFACE_CHECK_ONLY)
		return 0;

	ret = clk_set_rate(host->clk, rate);
printk(KERN_ERR "**mxc_nand_v2_setup_data_interface() ret=%d\n", ret);////debug
	if (ret)
		return ret;
...
printk(KERN_ERR "**mxc_nand_v2_setup_data_interface() good end\n");////debug
	return 0;
}
				...
				int column, int page_addr)
{
	struct nand_chip *nand_chip = mtd_to_nand(mtd);
	struct mxc_nand_host *host = nand_get_controller_data(nand_chip);
//printk(KERN_ERR "**mxc_nand_command()\n");////debug
...
//printk(KERN_ERR "**mxc_nand_command() end\n");////debug
}
...
static int mxcnd_probe(struct platform_device *pdev)
{
	...
	struct resource *res;
	int err = 0;
printk(KERN_ERR "**mxc_probe()\n");////debug
	/* Allocate memory for MTD device structure and private data */
	host = devm_kzalloc(&pdev->dev, sizeof(struct mxc_nand_host),
			GFP_KERNEL);
	if (!host)
		return -ENOMEM;
	//printk(KERN_ERR "**mxc_probe() host=%xh\n", (unsigned int)host);////debug - c323a820h
	...
	this = &host->nand;
	//printk(KERN_ERR "**mxc_probe() host=%ph, nand_chip=%xh\n", host, (unsigned int)this);////debug - c323a820h
	//for(err = 0;err<sizeof(struct nand_chip);err++){
	//printk(KERN_ERR "**mxc_probe() err=%d, nand_chip=%ph\n", err, &host->nand+err);////debug
		//*(unsigned char*)(this+err) = err;
	//}
...
printk(KERN_ERR "**mxc_probe() DRIVER_NAME=%s\n", DRIVER_NAME);////debug
printk(KERN_ERR "**mxc_probe() host->clk name=%s\n", pdev->name);////debug
printk(KERN_ERR "**mxc_probe() host->clk init_name=%s\n", pdev->dev.init_name);////debug
	//host->clk = devm_clk_get(&pdev->dev, NULL);////debug commented
	host->clk = devm_clk_get(&pdev->dev, "per");////debug
	////debug start
	if (IS_ERR(host->clk)){
printk(KERN_ERR "**mxc_probe() host->clk IS_ERR=%d\n", IS_ERR(host->clk));////debug
printk(KERN_ERR "**mxc_probe() host->clk PTR_ERR=%ld\n", PTR_ERR(host->clk));////debug
	}
	////debug end
	if (IS_ERR(host->clk))
...
printk(KERN_ERR "**mxc_probe() err=%d\n", err);////debug
	if (err < 0)
		return err;
...
printk(KERN_ERR "**mxc_probe() host->irq=%d\n", host->irq);////debug
	if (host->irq < 0)
		return host->irq;
...
printk(KERN_ERR "**mxc_probe() clk_prepare_enable err=%d\n", err);////debug
	if (err)
		return err;
	host->clk_act = 1;

printk(KERN_ERR "**mxc_probe() irqdepending_quirtk=%d\n", host->devtype_data->irqpending_quirk);////debug
	/*
...
printk(KERN_ERR "**mxc_probe() nand_scan_ident err=%d\n", err);////debug
	if (err)
		goto escan;

printk(KERN_ERR "**mxc_probe() nand_scan_ident err.mode=%d\n", this->ecc.mode);////debug
...
printk(KERN_ERR "**mxc_probe() nand_scan_ident bbt_options=%xh\n", this->bbt_options);////debug
	if (this->bbt_options & NAND_BBT_USE_FLASH) {
		this->bbt_td = &bbt_main_descr;
		this->bbt_md = &bbt_mirror_descr;
	}

printk(KERN_ERR "**mxc_probe() nand_scan_ident data_buf=%xh\n", (unsigned int)host->data_buf);////debug
	/* allocate the right size buffer now */
	devm_kfree(&pdev->dev, (void *)host->data_buf);
	host->data_buf = devm_kzalloc(&pdev->dev, mtd->writesize + mtd->oobsize,
					GFP_KERNEL);
printk(KERN_ERR "**mxc_probe() nand_scan_ident new data_buf=%xh\n", (unsigned int)host->data_buf);////debug
	if (!host->data_buf) {
		err = -ENOMEM;
		goto escan;
	}

printk(KERN_ERR "**mxc_probe() nand_scan_ident new preset\n");////debug
...
printk(KERN_ERR "**mxc_probe() nand_scan_ident oobsize=%d\n", host->used_oobsize);////debug
...
printk(KERN_ERR "**mxc_probe() nand_scan_tail\n");////debug
	/* second phase scan */
	err = nand_scan_tail(mtd);
printk(KERN_ERR "**mxc_probe() nand_scan_tail err=%d\n", err);////debug
	if (err)
		goto escan;
...
printk(KERN_ERR "**mxc_probe() good end\n");////debug
	return 0;
...
printk(KERN_ERR "**mxc_probe() escan err=%d\n", err);////debug
	return err;
}
