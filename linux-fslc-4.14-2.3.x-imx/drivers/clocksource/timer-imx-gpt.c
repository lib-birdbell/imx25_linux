...
{
	unsigned int tmp;
early_print("**imx1_gpt_irq_enable() imx21_gpt...%xh, %xh\n", (unsigned int)imxtm->base, MXC_TCTL);////debug
	tmp = readl_relaxed(imxtm->base + MXC_TCTL);
	writel_relaxed(tmp | MX1_2_TCTL_IRQEN, imxtm->base + MXC_TCTL);
}
...
{
	/*
	u64 temp = readl_relaxed(sched_clock_reg);////debug
	printk(KERN_ERR "**mxc_read_sched_clock() %llu\n", temp);
	return temp;*/
	return sched_clock_reg ? readl_relaxed(sched_clock_reg) : 0;////debug commented
}
...
static int __init mxc_clocksource_init(struct imx_timer *imxtm)
{
	...
	//printk(KERN_ERR "**mxc_clocksource_init() reg_tcn=%xh\n", imxtm->gpt->reg_tcn);////debug - 0x24h GPTCNT
	//struct clk *pClk = imxtm->clk_per;////debug
printk(KERN_ERR "**mxc_clocksource_init() c=%u Hz\n", c);////debug
//early_print("**mxc_clocksource_init() size=%dh\n", sizeof(imxtm->clk_per->core));////debug
//early_print("**mxc_clocksource_init() size=%d\n", sizeof(pClk->core));////debug
//early_print("**mxc_clocksource_init() get_name=%s\n", __clk_get_name(imxtm->clk_per));////debug
printk(KERN_ERR "**mxc_clocksource_init() get_rate=%d\n", clk_get_rate(imxtm->clk_per));////debug
//early_print("**mxc_clocksource_init() clk_per.get_name=%s\n", imxtm->clk_per->core->name);////debug
//early_print("**mxc_clocksource_init() clk_per.dev_id=%s\n", imxtm->clk_per->dev_id);////debug
//early_print("**mxc_clocksource_init() clk_per.con_id=%s\n", imxtm->clk_per->con_id);////debug
//early_print("**mxc_clocksource_init() clk_per.min_rate=%d\n", imxtm->clk_per->min_rate);////debug
//early_print("**mxc_clocksource_init() clk_per.max_rate=%d\n", imxtm->clk_per->max_rate);////debug
//c = 13300000;////debug
	if (c == 0)
		return -EINVAL;
...
//early_print("**mxc_lcocksource_init() reg=%xh\n", (unsigned int)reg);////debug
	sched_clock_reg = reg;
	//printk(KERN_ERR "**mxc_clocksource_init() sched_clock_reg=%xh\n", sched_clock_reg);////debug - f5790024h
...
imxtm->irq += 16;////debug
	...
	ced->irq = imxtm->irq;
	early_print("**mxc_clockevent_init() clk_per=%d\n", clk_get_rate(imxtm->clk_per));////debug
	clockevents_config_and_register(ced, clk_get_rate(imxtm->clk_per),
					0xff, 0xfffffffe);
printk(KERN_ERR "**mxc_clockevent_init() reg_tstat=%xh\n", imxtm->gpt->reg_tstat);////debug
	act->name = "i.MX Timer Tick";
	act->flags = IRQF_TIMER | IRQF_IRQPOLL;
	act->handler = mxc_timer_interrupt;
	act->dev_id = ced;
early_print("**mxc_clockevnet_init() irq=%d\n", imxtm->irq);////debug
...
{
	u32 tctl_val;
early_print("**imx1_gpt_setup_tctl() imx21_gpt...\n");////debug
...
static void imx25_gpt_setup_tctl(struct imx_timer *imxtm)
{
	u32 tctl_val;
...
static const struct imx_gpt_data imx25_gpt_data = {
	.reg_tstat = V2_TSTAT,
	//.reg_tcn = MX1_2_TCN,
	//.reg_tcmp = MX1_2_TCMP,
	.reg_tcn = V2_TCN,
	...
	.gpt_setup_tctl = imx25_gpt_setup_tctl,
	//.set_next_event = mx1_2_set_next_event,
...
early_print("**_mxc_timer_init() imxtm->base=%xh, %xh, %xh\n", (unsigned int)imxtm->base, MXC_TCTL, MXC_TPRER);////debug
early_print("**_mxc_timer_init() imxtm->type=%d(GPT_TYPE_IMX25=%d)\n", imxtm->type, GPT_TYPE_IMX25);////debug
	switch (imxtm->type) {
	case GPT_TYPE_IMX1:
	...
	case GPT_TYPE_IMX21:
		...
		break;
	case GPT_TYPE_IMX25:	////debug start added by LSJ 20220518
		imxtm->gpt = &imx25_gpt_data;
		break;		////debug end
	case GPT_TYPE_IMX31:
...
early_print("**_mxc_timer_init() imxtm->clk_per=%ph\n", imxtm->clk_per);////debug
	if (IS_ERR(imxtm->clk_per)) {
		early_print("**_mxc_timer_init() i.MX timer: unable to get clk\n");////debug
...
early_print("**_mxc_timer_init() ret=%d\n", ret);////debug
	if (ret)
		return ret;

early_print("**_mxc_timer_init() end\n");////debug
...
early_print("**mxc_timer_init() pbase=%xh\n", (unsigned int)pbase);////debug
...
early_print("**mxc_timer_init() end\n");////debug
}
